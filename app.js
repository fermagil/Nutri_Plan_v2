...
nuevoClienteBtn.addEventListener('click', () => {
    ...
    ['somatotype-point-canvas', 'typology-chart', 'weight-chart'].forEach(canvasId => {
        ...
        if (canvas) {
            ...
            // Biochemical parameters configuration
                const bioquimicosParams = [
                    {
                        subsection: 'Nutrición',
                        params: [
                            { key: 'albumina', label: 'Albúmina (g/dL)', min: 0, max: 10, step: 0.1 },
                            { key: 'prealbumina', label: 'Prealbúmina (mg/dL)', min: 0, max: 100, step: 0.1 }
                        ]
                    },
                    {
                        subsection: 'Lípidos',
                        params: [
                            { key: 'colesterolTotal', label: 'Colesterol Total (mg/dL)', min: 0, max: 500, step: 1 },
                            { key: 'hdl', label: 'HDL (mg/dL)', min: 0, max: 200, step: 1 },
                            { key: 'trigliceridos', label: 'Triglicéridos (mg/dL)', min: 0, max: 1000, step: 1 }
                        ]
                    },
                    {
                        subsection: 'Glucosa',
                        params: [
                            { key: 'glucosaAyunas', label: 'Glucosa en ayunas (mg/dL)', min: 0, max: 500, step: 1 },
                            { key: 'hba1c', label: 'HbA1c (%)', min: 0, max: 20, step: 0.01 },
                            { key: 'insulina', label: 'Insulina (µU/mL)', min: 0, max: 100, step: 0.1 }
                        ]
                    },
                    {
                        subsection: 'Inflamación',
                        params: [
                            { key: 'pcrUltrasensible', label: 'PCR ultrasensible (mg/L)', min: 0, max: 50, step: 0.01 },
                            { key: 'leptina', label: 'Leptina (ng/mL)', min: 0, max: 100, step: 0.1 }
                        ]
                    },
                    {
                        subsection: 'Hígado',
                        params: [
                            { key: 'alt', label: 'ALT (U/L)', min: 0, max: 500, step: 1 },
                            { key: 'ggt', label: 'GGT (U/L)', min: 0, max: 500, step: 1 }
                        ]
                    },
                    {
                        subsection: 'Hormonas',
                        params: [
                            { key: 'tsh', label: 'TSH (µIU/mL)', min: 0, max: 50, step: 0.01 },
                            { key: 'testosterona', label: 'Testosterona (ng/dL)', min: 0, max: 2000, step: 0.1 }
                        ]
                    },
                    {
                        subsection: 'Óseo',
                        params: [
                            { key: 'vitaminaD', label: 'Vitamina D (ng/mL)', min: 0, max: 200, step: 0.1 }
                        ]
                    }
                ];
                
                // Function to create and show the biochemical parameters modal
                function setupBioquimicosModal() {
                    const input = document.getElementById('parametros-bioquimicos');
                    const modal = document.getElementById('bioquimicos-modal');
                    const formContainer = document.getElementById('bioquimicos-form');
                    const closeButton = modal.querySelector('.close-button');
                
                    if (!input || !modal || !formContainer || !closeButton) {
                        console.error('Required modal elements not found');
                        return;
                    }
                
                    // Generate form content
                    const formHtml = bioquimicosParams.map(section => `
                    ...
                    formContainer.innerHTML = `
                        ${formHtml}
                        <div class="buttons">
                            <button class="save">Guardar Resultados</button>
                    ...
                    input.addEventListener('focus', () => {
                        ...
                        // Clear input to prevent text entry
                        input.blur();
                    });
                    ...
                    formContainer.querySelector('.cancel').addEventListener('click', closeModal);
                
                    // Save results
                    formContainer.querySelector('.save').addEventListener('click', () => {
                        const results = {};
                        let hasValidInput = false;
                
                        bioquimicosParams.forEach(section => {
                            section.params.forEach(param => {
                                const input = document.getElementById(param.key);
                                const value = input.value.trim();
                                if (value !== '') {
                                    const numValue = toNumber(value);
                                    if (numValue !== null && numValue >= param.min && numValue <= param.max) {
                                        results[param.key] = numValue;
                                        results[`${param.key}Source`] = 'Ingresado manualmente';
                                        hasValidInput = true;
                                    } else {
                                        alert(`Valor inválido para ${param.label}. Debe estar entre ${param.min} y ${param.max}.`);
                                        return;
                                    }
                                }
                            });
                        ...
                        if (hasValidInput) {
                            // Update resultElements and window.calculatedResults
                            Object.keys(results).forEach(key => {
                                if (resultMappings[key] && resultElements[key]) {
                                    const formattedValue = resultMappings[key].format(results[key]);
                                    resultElements[key].textContent = formattedValue;
                                    if (!window.calculatedResults) {
                                        window.calculatedResults = {};
                                    }
                                    window.calculatedResults[key] = formattedValue;
                                }
                            });
                            closeModal();
                        }
    ...
    try {
        ...
        if (data.resultados) {
            ...
            const resultMappings = {
                ...
                // Bioquímicos
                    'albumina': { id: 'result-albumina', unit: 'g/dL', format: (v) => typeof v === 'number' || (typeof v === 'string' && !isNaN(parseFloat(v))) ? toNumber(v).toFixed(1) : '---' },
                    'albuminaSource': { id: 'albumina-source', unit: '', format: (v) => v || '---' },
                    'prealbumina': { id: 'result-prealbumina', unit: 'mg/dL', format: (v) => typeof v === 'number' || (typeof v === 'string' && !isNaN(parseFloat(v))) ? toNumber(v).toFixed(1) : '---' },
                    'prealbuminaSource': { id: 'prealbumina-source', unit: '', format: (v) => v || '---' },
                    'colesterolTotal': { id: 'result-colesterol-total', unit: 'mg/dL', format: (v) => typeof v === 'number' || (typeof v === 'string' && !isNaN(parseFloat(v))) ? toNumber(v).toFixed(0) : '---' },
                    'colesterolTotalSource': { id: 'colesterol-total-source', unit: '', format: (v) => v || '---' },
                    'hdl': { id: 'result-hdl', unit: 'mg/dL', format: (v) => typeof v === 'number' || (typeof v === 'string' && !isNaN(parseFloat(v))) ? toNumber(v).toFixed(0) : '---' },
                    'hdlSource': { id: 'hdl-source', unit: '', format: (v) => v || '---' },
                    'trigliceridos': { id: 'result-trigliceridos', unit: 'mg/dL', format: (v) => typeof v === 'number' || (typeof v === 'string' && !isNaN(parseFloat(v))) ? toNumber(v).toFixed(0) : '---' },
                    'trigliceridosSource': { id: 'trigliceridos-source', unit: '', format: (v) => v || '---' },
                    'glucosaAyunas': { id: 'result-glucosa-ayunas', unit: 'mg/dL', format: (v) => typeof v === 'number' || (typeof v === 'string' && !isNaN(parseFloat(v))) ? toNumber(v).toFixed(0) : '---' },
                    'glucosaAyunasSource': { id: 'glucosa-ayunas-source', unit: '', format: (v) => v || '---' },
                    'hba1c': { id: 'result-hba1c', unit: '%', format: (v) => typeof v === 'number' || (typeof v === 'string' && !isNaN(parseFloat(v))) ? toNumber(v).toFixed(2) : '---' },
                    'hba1cSource': { id: 'hba1c-source', unit: '', format: (v) => v || '---' },
                    'insulina': { id: 'result-insulina', unit: 'µU/mL', format: (v) => typeof v === 'number' || (typeof v === 'string' && !isNaN(parseFloat(v))) ? toNumber(v).toFixed(1) : '---' },
                    'insulinaSource': { id: 'insulina-source', unit: '', format: (v) => v || '---' },
                    'pcrUltrasensible': { id: 'result-pcr-ultrasensible', unit: 'mg/L', format: (v) => typeof v === 'number' || (typeof v === 'string' && !isNaN(parseFloat(v))) ? toNumber(v).toFixed(2) : '---' },
                    'pcrUltrasensibleSource': { id: 'pcr-ultrasensible-source', unit: '', format: (v) => v || '---' },
                    'leptina': { id: 'result-leptina', unit: 'ng/mL', format: (v) => typeof v === 'number' || (typeof v === 'string' && !isNaN(parseFloat(v))) ? toNumber(v).toFixed(1) : '---' },
                    'leptinaSource': { id: 'leptina-source', unit: '', format: (v) => v || '---' },
                    'alt': { id: 'result-alt', unit: 'U/L', format: (v) => typeof v === 'number' || (typeof v === 'string' && !isNaN(parseFloat(v))) ? toNumber(v).toFixed(0) : '---' },
                    'altSource': { id: 'alt-source', unit: '', format: (v) => v || '---' },
                    'ggt': { id: 'result-ggt', unit: 'U/L', format: (v) => typeof v === 'number' || (typeof v === 'string' && !isNaN(parseFloat(v))) ? toNumber(v).toFixed(0) : '---' },
                    'ggtSource': { id: 'ggt-source', unit: '', format: (v) => v || '---' },
                    'tsh': { id: 'result-tsh', unit: 'µIU/mL', format: (v) => typeof v === 'number' || (typeof v === 'string' && !isNaN(parseFloat(v))) ? toNumber(v).toFixed(2) : '---' },
                    'tshSource': { id: 'tsh-source', unit: '', format: (v) => v || '---' },
                    'testosterona': { id: 'result-testosterona', unit: 'ng/dL', format: (v) => typeof v === 'number' || (typeof v === 'string' && !isNaN(parseFloat(v))) ? toNumber(v).toFixed(1) : '---' },
                    'testosteronaSource': { id: 'testosterona-source', unit: '', format: (v) => v || '---' },
                    'vitaminaD': { id: 'result-vitamina-d', unit: 'ng/mL', format: (v) => typeof v === 'number' || (typeof v === 'string' && !isNaN(parseFloat(v))) ? toNumber(v).toFixed(1) : '---' },
                    'vitaminaDSource': { id: 'vitamina-d-source', unit: '', format: (v) => v || '---' }
            };
